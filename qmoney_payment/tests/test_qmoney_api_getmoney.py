...
from helpers import QMoneyBearerAuth, set_into, del_from
...
class TestQmoneyAPIGetMoney:
    ...
    def test_failing_at_initiating_transaction_when_wrong_access_token(
        ...
        response = requests.post(
            url=f'{qmoney_url}/getMoney',
            json=json_payload,
            auth=QMoneyBearerAuth(
                'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJsb2dnaW5nQXMiOm51bGwsImF1ZCI6WyJBZGFwdGVyX09hdXRoX1Jlc291cmNlX1NlcnZlciJdLCJncmFudF90eXBlIjoicGFzc3dvcmQiLCJkZXZpY2VVbmlxdWVJZCI6bnVsbCwidXNlcl9uYW1lIjoiMTQwMDE1MDIiLCJzY'
            ))
        assert response.status_code == 401
        json_response = response.json()
