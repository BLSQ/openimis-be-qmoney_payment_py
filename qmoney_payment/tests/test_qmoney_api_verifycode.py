...
class TestQmoneyAPIVerifyCode:
  ...
  def transaction_id_and_otp(self, request, qmoney_access_token, qmoney_url,
    ...
    before_initiating_transaction = datetime.now(tz=timezone(timedelta(
        hours=1)))
  ...
  def test_proceeding_transaction(self, qmoney_access_token, qmoney_url,
    ...
    wallet = next((wallet for wallet in json_response['data']['balanceData']
  ...
  def test_proceeding_transaction_with_too_big_amount(self,
    ...
    json_response = response.json()
    assert json_response['data']['transactionId'] == transaction_id
    assert json_response['responseCode'] == '-120008'
  ...
  def test_proceeding_transaction_twice(self, qmoney_access_token, qmoney_url,
                                        transaction_id_and_otp):
    transaction_id, otp = transaction_id_and_otp
